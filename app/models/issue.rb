  ...
  acts_as_searchable :columns => ['subject', "#{table_name}.description"],
                     :preload => [:project, :status, :tracker],
										 :scope => lambda { |options|
												...
												r = r.where("(#{table_name}.author_id=? OR #{table_name}.assigned_to_id=? OR #{table_name}.id IN (SELECT journalized_id FROM journals where journalized_type='Issue' AND user_id=? GROUP BY journalized_id))", User.current.id, User.current.id, User.current.id) if options[:issues_involved]
												r = r.where("(#{table_name}.author_id=? OR #{table_name}.assigned_to_id=? OR #{table_name}.id IN (SELECT watchable_id FROM watchers WHERE user_id=? AND watchable_type = 'Issue') OR #{table_name}.id IN (SELECT journalized_id FROM journals where journalized_type='Issue' AND user_id=? GROUP BY journalized_id))", User.current.id, User.current.id, User.current.id, User.current.id) if options[:issues_involved_or_watched]
												r
											}
